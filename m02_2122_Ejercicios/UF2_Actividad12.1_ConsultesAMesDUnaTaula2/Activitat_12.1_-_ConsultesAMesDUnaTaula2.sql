...
##### Consultes a més d’una taula II #####

-- Tasca 1. Mostra els productes que no siguin postres.

SELECT *
	FROM producte p
	LEFT JOIN postre pos ON pos.id_producte = p.id_producte
    WHERE pos.id_producte IS NULL;

-- Tasca 2. Mostra tots els productes i la quantitat d'ingredients que tenen. 
-- Els productes que no son pizzes en si mateixos són un únic ingredient.
    
SELECT p.nom, COUNT(*) AS numIngrediente
	FROM producte p
    LEFT JOIN pizza piz ON piz.id_producte = p.id_producte
    LEFT JOIN pizza_ingredient pizing ON pizing.id_producte = piz.id_producte
    GROUP BY p.nom
    ORDER BY numIngrediente DESC;

-- Tasca 3. Mostra la quantitat de pizzes, begudes i postres que hem venut.

SELECT p.nom, SUM(cp.quantitat) AS cantidadVendida
	FROM comanda_producte cp
		INNER JOIN producte p ON p.id_producte = cp.id_producte
	GROUP BY p.nom;

-- Tasca 4. Afegeix l'ingredient 'CEB', 'Ceba'. 
-- Mostra tots els ingredients i la quantitat que fem servir a les pizzes.

INSERT INTO ingredient (id_ingredient, nom) VALUE ('CEB', 'Ceba');

SELECT ing.nom, COUNT(pizing.id_ingredient)
	FROM ingredient ing
		LEFT JOIN pizza_ingredient pizing ON pizing.id_ingredient = ing.id_ingredient
        LEFT JOIN pizza piz ON piz.id_producte = pizing.id_producte
        LEFT JOIN producte p ON p.id_producte = piz.id_producte
	GROUP BY ing.nom;
