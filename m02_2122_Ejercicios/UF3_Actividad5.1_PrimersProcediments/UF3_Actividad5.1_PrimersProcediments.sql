...
DELIMITER //
CREATE PROCEDURE allFromClients()
BEGIN
    SELECT * 
		FROM client;
END //
...
DELIMITER //
CREATE PROCEDURE sumaPostBeb()
BEGIN
	SELECT COUNT(p.id_producte)
		FROM producte p
			LEFT JOIN beguda be ON be.id_producte = p.id_producte
			LEFT JOIN postre pos ON pos.id_producte = p.id_producte
	WHERE p.id_producte = be.id_producte OR p.id_producte = pos.id_producte;
END //
DELIMITER ;

-- VARIABLE
DELIMITER //
CREATE PROCEDURE sumaPostBeb2()
BEGIN
	SET @nBe = (SELECT count(*) FROM beguda);
    SET @nPos = (SELECT count(*) FROM postre);
    SET @total = @nBe + @nPos;
    SELECT @total;
END //
DELIMITER ;
...
-- Tasca 4. Actualitza el preu de tots els productes amb un increment del 1,5%. 
-- A continuació crida també al procediment de la Tasca 3.
...
DROP PROCEDURE IF EXISTS sumaPostBeb;
DROP PROCEDURE IF EXISTS sumaPostBeb2;

-- Tasca 6. Mostra la informació d’execució del procediment de la Tasca 3.

SHOW CREATE PROCEDURE nomPreu;

-- Tasca 7. Mostra sense usar un procediment tots els procediments generats a la 
-- taula del sistema de mysql.
